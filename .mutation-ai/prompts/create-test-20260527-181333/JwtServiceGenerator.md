...
CRITICAL — JWT token testing: when testing extractUsername, extractClaim, isTokenValid:
- Do NOT create tokens manually with Jwts.builder() — the internal key derivation may differ.
- ALWAYS create test tokens using the service's own `subject.generateToken(user)` method.
  Flow: `String token = subject.generateToken(user); subject.extractUsername(token);`
- The `generateToken` call on the SAME `subject` instance uses the SAME key → tokens are valid.
