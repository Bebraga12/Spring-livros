...
- For @Bean methods returning framework objects (AuthenticationProvider, PasswordEncoder, etc.):
  call the method on `subject`, assert the result is not null and is an instance of the expected type.
  Example: `assertNotNull(subject.passwordEncoder()); assertTrue(subject.passwordEncoder() instanceof BCryptPasswordEncoder);`
- For @Bean methods that accept a parameter (e.g. AuthenticationConfiguration): mock the parameter with `mock(AuthenticationConfiguration.class)`,
  stub `when(config.getAuthenticationManager()).thenReturn(mock(AuthenticationManager.class))`,
  then assert the returned value is not null.
- For @Bean UserDetailsService: call subject.userDetailsService() to get the lambda, then
  test loadUserByUsername() by stubbing the repository mock — use the EXACT entity class from the repository.
