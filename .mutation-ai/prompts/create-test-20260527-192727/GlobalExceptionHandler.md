...
CRITICAL — @ControllerAdvice handler type mapping: each handler takes a SPECIFIC exception type.
  NEVER pass the wrong exception type to a handler. Match the @ExceptionHandler annotation:
  subject.handle01(methodArgumentNotValidException) — MethodArgumentNotValidException ONLY
  subject.handle02(constraintViolationException)   — ConstraintViolationException ONLY
  subject.handle03(anyException)                   — Exception ONLY

EXACT PATTERN for handle01 (@ExceptionHandler(MethodArgumentNotValidException.class)):
    @Mock private MethodArgumentNotValidException mave;
    // In test:
    ...
    when(fe.getField()).thenReturn("field"); when(fe.getDefaultMessage()).thenReturn("error");
    when(br.getFieldErrors()).thenReturn(java.util.List.of(fe));
    when(mave.getBindingResult()).thenReturn(br);  // stub BindingResult SEPARATELY
    ResponseEntity<?> r = subject.handle01(mave);  // pass mave (MethodArgumentNotValidException)
    assertEquals(HttpStatus.BAD_REQUEST, r.getStatusCode());
    assertEquals("error", ((java.util.Map<?,?>)r.getBody()).get("field"));

EXACT PATTERN for handle02 (@ExceptionHandler(ConstraintViolationException.class)):
    @Mock private ConstraintViolationException cve;
    // In test — use RETURNS_DEEP_STUBS to allow chained calls:
    jakarta.validation.ConstraintViolation<?> v = mock(jakarta.validation.ConstraintViolation.class, Answers.RETURNS_DEEP_STUBS);
    when(v.getPropertyPath().toString()).thenReturn("field");  // safe with RETURNS_DEEP_STUBS
    when(v.getMessage()).thenReturn("error");
    when(cve.getConstraintViolations()).thenReturn(java.util.Set.of(v));
    ResponseEntity<?> r = subject.handle02(cve);  // pass cve (ConstraintViolationException)
    assertEquals(HttpStatus.BAD_REQUEST, r.getStatusCode());
    assertEquals("error", ((java.util.Map<?,?>)r.getBody()).get("field"));

  NEVER do: `when(violation.getPropertyPath()).thenReturn(mock(Path.class))` (no toString stub — wrong key!)
  NEVER do: `when(ex.getBindingResult().getFieldErrors())...` — NPE!
  NEVER do: `new FieldError(..., violation)` — FieldError takes String args only.
